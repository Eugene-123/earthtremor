...
cur.execute("SELECT * FROM earthquake WHERE mag > 5.0")
...
def search():
    conn = sqlite3.connect('earthquake.db')
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM earthquake WHERE mag > 5.0")
    count = c.fetchone()[0]
    conn.close()
...
def week():
    ...
    conn = sqlite3.connect('earthquake.db')
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM earthquake WHERE mag >= ? AND time >= ? AND time < ?", (magnitude, start_date, end_date))
    count = c.fetchone()[0]
    conn.close()
